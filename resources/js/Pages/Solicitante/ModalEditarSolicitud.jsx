...
export default function ModalEditarSolicitud({ abierto, onCerrar, solicitud }) {
    ...
    const { data, setData, put, processing, reset } = useForm({
     ...
     useEffect(() => {
        if (solicitud) {
            setData({
                nombre_departamento_solicitante: solicitud.departamento ?? '',
                nombre_solicitante:              solicitud.nombre_solicitante ?? '',
                descripcion_equipo:              solicitud.descripcion_equipo ?? '',
    ...
    function handleSubmit(event) {
        ...
        put(route('solicitante.sol_mant.update', solicitud.acciones), {
            onSuccess: () => {
                reset();
                onCerrar();
            }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-blue-800">Editar Solicitud</h2>
